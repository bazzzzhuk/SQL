-- SQLQuery1 - GetNextLearnDay.sql

USE PV_521_Import;
SET DATEFIRST 1;
GO

CREATE OR ALTER FUNCTION GetNextLearnDay(@group_name	AS NCHAR(10), @last_date AS DATE = '1900-01-01') RETURNS TINYINT
AS
BEGIN
	DECLARE @group_id	AS	INT		=	(SELECT group_id	FROM Groups		WHERE group_name = @group_name);
	DECLARE @weekdays	AS	TINYINT	=	(SELECT weekdays	FROM Groups		WHERE group_name = @group_name);
	if @last_date = CAST('1900-01-01' AS DATE) SET @last_date	=	(SELECT MAX([date]) FROM Schedule	WHERE [group] = @group_id);
